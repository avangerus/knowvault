...
func TestWorkerMountOwnershipIsDistinct(t *testing.T) {
	for name, consumer := range map[string]runtimeidentity.MountConsumer{"server": runtimeidentity.Server, "worker": runtimeidentity.Worker} {
		t.Run(name, func(t *testing.T) {
			...
			path := filepath.Join(directory, searchManifestFilename)
			...
			for _, denied := range []runtimeidentity.MountConsumer{other, 255} {
				rejected, err := openSearchMountedRootForConsumer(directory, denied)
				if rejected != nil {
					_ = rejected.Close()
			...
			root, err := openSearchMountedRootForConsumer(directory, consumer)
			if err != nil {
				t.Fatal(err)
			}
			defer root.Close()
			data, err := root.read(searchManifestFilename, maximumManifestBytes)
