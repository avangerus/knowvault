...
func openSearchMountedRootForConsumer(path string, consumer runtimeidentity.MountConsumer) (*searchMountedRoot, error) {
	...
	return &searchMountedRoot{fd: fd, opened: true, group: group}, nil
}

func (root *searchMountedRoot) read(filename string, maxBytes int) ([]byte, error) {
	if root == nil || (filename != searchManifestFilename && filename != searchRootCAFilename && filename != searchClientCertFile && filename != searchClientKeyFile) ||
		(maxBytes != maximumManifestBytes && maxBytes != maximumRootBytes && maxBytes != maximumClientCertBytes) {
	...
	var info syscall.Stat_t
	if err := syscall.Fstat(fd, &info); err != nil || info.Mode&syscall.S_IFMT != syscall.S_IFREG || info.Uid != 0 || info.Gid != root.group || info.Nlink != 1 || info.Mode&0o7777 != 0o440 || info.Size < 1 || info.Size > int64(maxBytes) {
		return nil, &Error{code: CodeMountInvalid}
	}
	return stableSearchRead(fd, maxBytes, info)
}

func stableSearchRead(fd, maxBytes int, initial syscall.Stat_t) ([]byte, error) {
	...
	if len(contents) != int(initial.Size) || syscall.Fstat(fd, &final) != nil || !sameSearchSnapshot(initial, final) {
		return nil, &Error{code: CodeMountInvalid}
	}
	return contents, nil
}

func sameSearchSnapshot(initial, final syscall.Stat_t) bool {
...
func (root *searchMountedRoot) Close() error {
	if root == nil {
		return nil
	}
	root.mu.Lock()
