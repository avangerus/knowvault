...
// This file owns the administrator-provisioned embedding capability mount.
...
const (
	DefaultMountRoot          = "/run/knowvault/embedding"
	embeddingManifestFilename = "manifest.json"
	embeddingProfileFilename  = "profile.json"
	embeddingRootCAFilename   = "root-ca.pem"
	embeddingClientCertFile   = "client-cert.pem"
	embeddingClientKeyFile    = "client-key.pem"
	maximumManifestBytes      = 64 << 10
	maximumProfileBytes       = 64 << 10
	maximumRootBytes          = 1 << 20
	maximumClientCertBytes    = 256 << 10
	embeddingManifestSchema   = "knowvault-embedding-manifest-v1"
	maximumCertificates       = 256
)

const (
	// CodeMountUnavailable means the optional vector capability is not mounted.
	// Composition may keep ingestion/search alive with an explicit partial
	// channel, but it must never claim a complete hybrid answer.
	CodeMountUnavailable ErrorCode = "EMBEDDING_MOUNT_UNAVAILABLE"
	...
	CodeMountInvalid ErrorCode = "EMBEDDING_MOUNT_INVALID"
)
...
// MountedConfig is a complete, tenant-bound embedding capability. It is
...
func LoadWorkerMountedForTenant(organizationID string) (MountedConfig, error) {
...
func LoadWorkerMountedAt(rootPath, organizationID string) (MountedConfig, error) {
...
func loadMountedForConsumer(rootPath, organizationID string, consumer runtimeidentity.MountConsumer) (MountedConfig, error) {
	...
	root, err := openEmbeddingMountedRootForConsumer(rootPath, consumer)
	if err != nil {
	...
	defer root.Close()
	manifestBytes, err := root.read(embeddingManifestFilename, maximumManifestBytes)
	if err != nil {
		if CodeOf(err) == CodeMountInvalid {
			return MountedConfig{}, &Error{code: CodeMountInvalid}
		}
	...
	roots, err := parseEmbeddingRoots(rootBytes)
	if err != nil {
		return MountedConfig{}, &Error{code: CodeMountInvalid}
	}
...
func parseMountedManifest(raw []byte, organizationID string) (mountedManifest, error) {
	...
	if manifest.Schema != embeddingManifestSchema || manifest.OrganizationID != organizationID ||
		manifest.ProfileFile != embeddingProfileFilename || !validSHA256(manifest.ProfileHash) ||
		manifest.RootCAFile != embeddingRootCAFilename ||
		manifest.ClientCertificateFile != embeddingClientCertFile || manifest.ClientKeyFile != embeddingClientKeyFile {
		return mountedManifest{}, &Error{code: CodeMountInvalid}
	}
	return manifest, nil
}

func parseEmbeddingRoots(raw []byte) (*x509.CertPool, error) {
	if len(raw) == 0 || len(raw) > maximumRootBytes {
		return nil, errors.New("embedding roots unavailable")
	}
	...
	for {
		...
		if block == nil || block.Type != "CERTIFICATE" || len(block.Headers) != 0 || len(block.Bytes) == 0 {
			return nil, errors.New("invalid embedding root")
		}
		certificate, err := x509.ParseCertificate(block.Bytes)
		if err != nil || !validEmbeddingCertificate(certificate) {
			return nil, errors.New("invalid embedding root")
		}
		digest := sha256.Sum256(block.Bytes)
		if _, duplicate := seen[digest]; duplicate {
			return nil, errors.New("duplicate embedding root")
		}
		seen[digest] = struct{}{}
		pool.AddCert(certificate)
		count++
		if count > maximumCertificates {
			return nil, errors.New("too many embedding roots")
		}
		rest = remainder
	}
	if count == 0 || len(pool.Subjects()) == 0 {
		return nil, errors.New("empty embedding roots")
	}
	return pool, nil
}

func validEmbeddingCertificate(value *x509.Certificate) bool {
