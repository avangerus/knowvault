...
// Real PostgreSQL publication retains a sentinel while one stable SQL identity
// disappears, returns unchanged, then disappears and returns changed.
func TestPostgreSQLAbsenceReturnPreservesAuthorityAndHistory(t *testing.T) {
	...
	history := []retained{}
	previous := ""
	for i, amount := range []string{"100.000", "100.000", "", "100.000", "100.000", "", "200.000", "200.000"} {
		rowAmount := amount
		if rowAmount == "" {
			rowAmount = previous
		}
		label := fmt.Sprintf("return-%d", i)
		row, err := postgresqlquery.CanonicalizeRow(projection.Columns, []any{"550e8400-e29b-41d4-a716-000000000001", "2026-08-01T12:34:56.789Z", rowAmount, "region-00"})
		if err != nil {
			t.Fatal(err)
		}
		sentinel, err := postgresqlquery.CanonicalizeRow(projection.Columns, []any{"550e8400-e29b-41d4-a716-000000000002", "2026-08-01T12:34:56.789Z", "1.000", "sentinel"})
		if err != nil {
			t.Fatal(err)
		}
		rows := []postgresqlquery.Row{sentinel}
		if amount != "" {
		...
		snapshot := postgresqlquery.Snapshot{Rows: rows, RowCount: len(rows), CoverageComplete: true, SnapshotHash: hash}
		...
		identity, err := postgresqlquery.IdentityDigest(s1dDigestKey, 1, projection, row)
		if err != nil {
			t.Fatal(err)
		}
		if amount == "" {
			if result.RowsReconciled != 1 {
				t.Fatalf("absent row reconciliation=%d", result.RowsReconciled)
			}
			for _, old := range history {
				read, err := viewer.ReadObjectExactVersion(ctx, regOwnerAccess("req_absent_read"), binding.workspaceID, old.fragment, old.version)
				if !errors.Is(err, evidence.ErrNotFound) || len(read.Text) != 0 {
					t.Fatalf("missing SQL row leaked history: %v", err)
				}
			}
			var objectState, memberState string
			if err := admin.QueryRow(ctx, `SELECT o.lifecycle_state,m.membership_state FROM public.source_object o JOIN public.source_object_scope m ON m.organization_id=o.organization_id AND m.source_object_id=o.id WHERE o.organization_id=$1 AND o.external_object_id_digest=$2`, regOrg, identity).Scan(&objectState, &memberState); err != nil {
				t.Fatal(err)
			}
			if objectState != "MISSING" || memberState != "MISSING" {
				t.Fatalf("missing SQL row state=%s/%s", objectState, memberState)
			}
			continue
		}
		var version, fragment, actualHash string
		if err := admin.QueryRow(ctx, `SELECT v.id,f.id,v.content_hash FROM public.source_object o JOIN public.source_version v ON v.organization_id=o.organization_id AND v.id=o.current_version_id JOIN public.evidence_fragment f ON f.organization_id=v.organization_id AND f.source_version_id=v.id WHERE o.organization_id=$1 AND o.connection_id=$2 AND o.external_object_id_digest=$3 ORDER BY f.ordinal LIMIT 1`, regOrg, registered.ConnectionID, identity).Scan(&version, &fragment, &actualHash); err != nil {
			t.Fatal(err)
		}
		...
		if amount != previous {
			if result.VersionsCreated != 1 && !(i == 0 && result.VersionsCreated == 2) || result.EvidencePublished == 0 {
		...
		var count int
		if err := admin.QueryRow(ctx, `SELECT count(*) FROM public.source_version WHERE organization_id=$1 AND source_object_id=(SELECT id FROM public.source_object WHERE organization_id=$1 AND external_object_id_digest=$2)`, regOrg, identity).Scan(&count); err != nil {
			t.Fatal(err)
		}
